...
function Navbar() {
  let [move,setMove]=useState("top-[-100vh]");
  let [disp,setDisp]=useState("lg:hidden");
  let [disp1,setDisp1]=useState("hidden");
  var flag=0;
  let niche=()=>{
  if(flag==0){
    setMove("top-[0]");
  ...
  return (
    <>
    <div className={`resp w-full h-[100vh] bg-zinc-700 lg:hidden fixed z-[11] ${move} transition-all ease-in-out duration-500 pt-[20vh]`}>
      <div className='w-full border-t-2 border-white px-[20px]'>
        {["services","our work","about us","insights","contact us"].map((item,index)=>
        <h1 className='text-white uppercase text-[7vw] font-bold' key={index}>{item}</h1>)}
      </div>
    </div>
    <div className='navbar w-full text-white py-7 px-12  flex justify-between fixed z-[12]'>
      ...
      <div className={'navbar2 flex gap-10 text-md'}>
                  {["Services","Our Work","About us","Insights","Contact us"].map((item,index)=>(<a key={index} className={`${index===4 && 'ml-60'}`}>{item}</a>))}
                  
      </div>
      <CiMenuFries onClick={()=>niche()} className={`menu md:text-[3vw] lg:hidden ${disp}`} />
      <RxCross2 onClick={()=>niche()} className={`menu1 md:text-[3vw] ${disp1}`} />
